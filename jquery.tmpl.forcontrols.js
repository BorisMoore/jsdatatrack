 ...
 * EXPERIMENTAL VERSION OF JQUERY TEMPLATES, WITH A CONCEPT OF CONTROLS ASSOCIATING CODE WITH TEMPLATES.
...
(function( jQuery, undefined ){
	...
	jQuery.each({
		...
		jQuery.fn[ name ] = function( selector ) {
			...
			jQuery.tmpl.complete( tmplItems, ctl, newData );
			ctl = null;
			newData = null;
			return ret;
		};
	...
	jQuery.fn.extend({
		...
		template: function( name, options ) {
			return jQuery.template( name, this[0], options );
		},
		...
		domManip: function( args, table, callback, options ) {
			...
			if ( ctl && !appendToTmplItems ) { // Bug in current code
				jQuery.tmpl.complete( newTmplItems, ctl, newData );
			}
			return this;
		}
	});
	...
	jQuery.extend({
		...
		//	options =  jQuery.extend( {}, options, tmpl )
			...
			if ( options && options.wrapped ) {
				...
				if ( options.addIds ) {
					// TEMPORARY?
					tmpl = jQuery.template( null, options._wrap );
					options._wrap = null;
	...
	jQuery.extend( jQuery.tmpl, {
		...
		item: {
			...
			wrap: function( call, wrapped ) {
				...
				return jQuery.tmpl( jQuery.template( call[2] ), call[3], options, call[0] ); // tmpl, data, options, item
			},
			...
			pluginOLD: function( call, wrapped ) {
				...
				return $.tmpl( pluginsWrapperTmpl, null, {wrapped: wrapped, addIds: true, elementCreated: function( element ) {
					...
					if ( $.fn[ pluginName ] ) {
						...
						$.defer[ pluginName ].done( loadPlugin );
					}
					function loadPlugin() {
						var self = $( element );
			...
			plugin: function( call, wrapped ) {
				...
				return $.tmpl( pluginsWrapperTmpl, null, {wrapped: wrapped, addIds: true, elementCreated: function( element ) {
					...
					function loadPlugin() {
						var self = $( element );
						self[ pluginName ].apply( self, $.makeArray( call ).slice(4) );
						if ( call[2] ) {
				...
				function( all, slash, type, fnargs, target, parens, args ) {
					...
					if ( !tag ) {
						...
						//	open: defaultOpen, // This was for pluginOLD
							close: defaultClose.join( type )
						};
						jQuery.tmpl.item[ type ] = jQuery.tmpl.item[ type ] || function( call, wrapped ) {
							var pluginCall = $.makeArray( call );
							pluginCall.splice( 3, 0, type );
...
//								})
	...
	function storeTmplItems( content ) {
		...
		while ( elemCreated.length ) {
			...
			jQuery.data( elem, "tmplItem" ).elementCreated( elem );
		}
		function processItemKey( el ) {
			...
			function cloneTmplItem( key ) {
				...
				tmplItem = newClonedItems[key] = 
					(newClonedItems[key] || new newTmplItem( tmplItem, newTmplItems[tmplItem.parent.key + keySuffix] || tmplItem.parent, null, true ));
			}
		}
	}
})( jQuery );
